...
// Hàm helper để format số Big.js thành chuỗi đầy đủ Đéo có ký hiệu e+
function formatBigNumber(bigNum) {
  // Chuyển đổi sang chuỗi khoa học
  const scientificStr = bigNum.toString();

  // Nếu Đéo có ký hiệu e, format luôn với dấu chấm
  ...
  try {
    ...
    if (amount.endsWith('%')) {
      ...
      if (isNaN(percentage) || percentage <= 0 || percentage > 100) {
        throw new Error('Phần trăm cược Đéo hợp lệ (1-100%)');
      }
    ...
    // Xử lý các đơn vị tiền tệ
      ...
      if (isNaN(number)) {
        throw new Error('Số tiền Đéo hợp lệ');
      }
    ...
    throw new Error('Số tiền Đéo hợp lệ: ' + error.message);
  }
}
