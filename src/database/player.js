...
export async function login(username, password, idUserZalo, senderName, api) {
  try {
    ...
    if (accountRows.length === 0) {
      ...
      return {
        success: false,
        message: `Tên đăng nhập hoặc mật khẩu Đéo đúng. ${
...
export async function setLoserGame(idUser, amount) {
  try {
    ...
    if (playerRows.length === 0) {
      return { success: false, message: `${nameServer}: Đéo tìm thấy người chơi. ❌` };
    }
...
export async function getPlayerBalance(idUser) {
  try {
    ...
    if (rows.length > 0) {
      ...
      return {
        success: false,
        message: `Đéo tìm thấy dữ liệu người chơi của bạn, nếu chưa đăng nhập, hãy chat lệnh game để xem hướng dẫn!`,
      };
    }
  } catch (error) {
...
export async function updateAccountVND(username, amount) {
  try {
    ...
    if (currentBalance.length === 0) {
      return { success: false, message: `${nameServer}: Đéo tìm thấy tài khoản!` };
    }
...
export async function registerAccount(username, password) {
  try {
    ...
    if (specialChars.test(password) || vietnameseChars.test(password) || specialChars.test(username) || vietnameseChars.test(username)) {
      return {
        success: false,
        message: `${nameServer}: Tài khoản hoặc mật khẩu Đéo được chứa ký tự đặc biệt hoặc dấu. Vui lòng thử lại (lưu ý bỏ dấu [ và ])!`,
      };
    }
