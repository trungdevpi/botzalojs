...
export async function scanGroupsWithAction(api, message, groupInfo, aliasCommand) {
	...
	try {
		...
		if (searchResults.length) {
			...
			msg = `🔍 Đéo tìm thấy thành viên nào trong nhóm ${groupInfo.name} có ${VALID_ACTIONS[action]} với "${searchTerm}"!`;
		}
	...
	try {
		...
		if (scanData.userRequest !== message.data.uidFrom) {
			await sendMessageWarning(
				api,
				message,
				"Bạn Đéo phải người yêu cầu hành động này!",
				true
			);
			return true;
		}
		...
		if (!scanData.botIsAdminBox) {
			await sendMessageQuery(
				api,
				message,
				"Tôi Đéo đủ quyền để thực hiện hành động này!",
				true
			);
			return true;
		}
		...
		for (const member of scanData.results) {
			if (isAdmin(member.id, threadId)) {
				results.push(`${member.zaloName} -> Đéo thể ${action === "kick" ? "Kick" : "Block"} quản trị nhóm!`);
			} else {
		...
		if (action === "kick") {
			for (const member of uidFinal) {
				try {
					...
					results.push(`${member.zaloName} -> Đéo thể Kick do ${error.message}!`);
				}
			}
		} else {
			for (const member of uidFinal) {
				try {
					...
					results.push(`${member.zaloName} -> Đéo thể Block do ${error.message}!`);
				}
			}
		}
