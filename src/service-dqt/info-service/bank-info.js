...
async function createBankCardImage(bankInfo, accountNumber, accountName, amount, description, qrCodeUrl, userInfo) {
    ...
    try {
        ...
        const fields = [
            ...
            { label: "Số tiền:", value: amount ? amount.toLocaleString('vi-VN') + ' VNĐ' : 'Đéo có' },
            { label: "Nội dung:", value: description ? (description.length > 20 ? description.substring(0, 20) + '...' : description) : 'Đéo có' }
        ];
...
export async function handleBankInfoCommand(api, message, aliasCommand) {
    ...
    try {
        ...
        if (!accountNumber) {
            const result = {
                success: false,
                message: `Đéo tìm thấy số tài khoản trong nội dung.`
            };
            await sendMessageFromSQL(api, message, result, false, 15000);
            return;
        }
        ...
        if (!bankInfo) {
            ...
            const result = {
                success: false,
                message: `Đéo tìm thấy tên ngân hàng hợp lệ trong nội dung.${suggestionText}`
            };
            await sendMessageFromSQL(api, message, result, false, 15000);
            return;
        }
