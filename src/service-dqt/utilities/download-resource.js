...
export async function handleDownloadResource(api, message, aliasCommand) {
  ...
  if (args.length < 2) {
    const object = {
      caption: `Vui lòng nhập đúng cú pháp: ${prefix}${aliasCommand} [thư mục]|[tên file]|[link (nếu Đéo reply)]`,
    };
  ...
  if (!isValidDirectory(dirName)) {
    const object = {
      caption: `Thư mục "${dirName}" Đéo tồn tại trong resource!`,
    };
    await sendMessageWarningRequest(api, message, object, 30000);
    return;
  }
...
export async function handleDeleteResource(api, message, aliasCommand) {
  ...
  if (matchingFiles.length === 0) {
    const object = {
      caption: `Đéo tìm thấy file "${fileName}" trong thư mục "${dirName}"!`,
    };
    await sendMessageWarningRequest(api, message, object, 30000);
    return;
  }
