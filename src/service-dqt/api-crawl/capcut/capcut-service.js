...
// Author: ndqitvn
// Description: Code get data youtube by N C H (ndqitvn)
...
export const searchCapcut = async (query, limit = CONFIG.maxResults) => {
  try {
    ...
    if (!response?.data?.data?.video_templates) {
      throw new Error("Đéo thể lấy được dữ liệu từ Capcut");
    }
...
export async function handleCapcutCommand(api, message, aliasCommand) {
  ...
  try {
    ...
    if (searchResults.length === 0) {
      object.caption = `Đéo tìm thấy template phù hợp với cụm từ: ${keyword}`;
  ...
  await api.sendVideo({
    ...
    message: {
      text: `[ ${senderName} ]\n` +
        ...
        `⏱️ Thời Lượng: ${template.duration ? `${(template.duration / 1000).toFixed(0)}s` : "Đéo rõ"}\n` +
