...
// Author: SBT
// Description: Code get data youtube by N C H (SBT)
...
export const searchYouTube = async (query) => {
  try {
    ...
    if (!initialData) throw new Error("Đéo thể lấy được dữ liệu từ YouTube");
    ...
    if (videos.length === 0) {
      throw new Error("Đéo tìm thấy video nào");
    }
  ...
  try {
    ...
    if (url) {
      ...
      try {
        ...
        if (!videoInfo) {
          throw new Error("Đéo thể lấy thông tin video");
        }

        const durationMs = videoInfo.duration * 1000;
        if (!isAdminLevelHigh && durationMs > 30 * 60 * 1000) {
          const object = {
            caption: "Vì tài nguyên có hạn, Đéo thể lấy video có độ dài hơn 30 phút!\n" +
        ...
        if (cachedVideo) {
          ...
          try {
            ...
            if (!fs.existsSync(videoPath)) {
              throw new Error(`Đéo thể tải video : ${videoPath}`);
            }
    ...
    if (videos.length === 0) {
      const object = {
        caption: `Đéo tìm thấy video phù hợp với cụm từ: ${searchQuery}`
      };
      return await sendMessageWarningRequest(api, message, object, 30000);
    }
...
export async function handleYoutubeReply(api, message) {
  ...
  try {
    ...
    if (selectedIndex < 0 || selectedIndex >= collection.length) {
      const object = {
        caption: `Số bạn chọn Đéo nằm trong danh sách. Vui lòng chọn lại.`,
      };
      await sendMessageWarningRequest(api, message, object, 30000);
      return true;
    }
...
const validateYouTubeResponse = (response) => {
  if (!response?.data) {
    throw new Error("Đéo nhận được dữ liệu từ YouTube");
  }

  if (!response.data.includes("ytInitialData")) {
    throw new Error("Đéo phải trang kết quả tìm kiếm YouTube");
  }
...
const convertPublishedTimeToVietnamese = (publishedTime) => {
  if (!publishedTime) return "Đéo xác định";
...
const formatUploadDateToTimeAgo = (uploadDate) => {
  try {
    if (!uploadDate || uploadDate.length !== 8) return "Đéo xác định";
...
export const getYoutubeVideoInfo = async (videoUrl) => {
  try {
    ...
    console.error("Lỗi khi lấy thông tin video:", error);
    throw new Error("Đéo thể lấy thông tin video: " + error.message);
  }
};
