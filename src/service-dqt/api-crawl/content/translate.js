    ...
    if (parts.length > 1) {
      ...
      try {
        ...
        await api.sendMessage(
          {
            msg: "Vì vấn đề nào đó, tôi Đéo dịch được từ này.",
            quote: message,
          },
          threadId,
          message.type
...
async function translateWithGPT(text, targetLanguage) {
  const prompt = `Dịch giúp tôi cụm từ "${text}" sang ngôn ngữ ${targetLanguage}, chỉ đưa ra kết quả, Đéo giải thích gì thêm, nếu Đéo phải lating (tiếng Trung tiếng Hàn) thì thêm chú thích phiên âm`;
