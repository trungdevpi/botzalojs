...
export async function weatherCommand(api, message) {
  ...
  if (!location) {
    // Nếu Đéo có địa điểm, hiển thị thời tiết tổng quan
...
async function getLocalWeather(api, message, threadId, location, isOverall = false) {
  try {
    ...
    if (!geoData.results || geoData.results.length === 0) {
      await api.sendMessage(
        { msg: "Đéo tìm thấy thành phố. Vui lòng kiểm tra lại tên thành phố.", quote: message },
        threadId,
        message.type
      );
      return;
...
// Lấy dữ liệu từ OpenWeather (tốt cho thông tin hiện tại và chất lượng Đéo khí)
...
function formatWeatherInfo(name, admin1, country, tomorrow, openWeather, weatherApi, isOverall) {
  ...
  // Tính toán chỉ số UV và chất lượng Đéo khí
...
function getPrecipitationForecast(hourlyData) {
  if (!Array.isArray(hourlyData)) return "Đéo có dữ liệu dự báo mưa";
  ...
  if (!rainHour) {
    // Kiểm tra xem có khả năng mưa thấp Đéo
    ...
    return "Dự kiến Đéo có mưa trong 24 giờ tới";
  }
  ...
  try {
    ...
    return "Đéo thể dự đoán chính xác thời gian mưa";
  }
}

function getRainIntensity(intensity) {
  if (intensity === 0) return "Đéo mưa";
...
function getWeatherDescription(code) {
  ...
  return weatherCodes[code] || "Đéo rõ";
}
