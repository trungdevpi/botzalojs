...
const getClientId = async () => {
  try {
    ...
    if (!urls.length) {
      throw new Error("Đéo tìm thấy URL script");
    }
    ...
    console.error(`Đéo thể lấy client ID: ${error}`);
    try {
...
async function getMusicStreamUrl(link) {
  try {
    ...
    if (!progressiveUrl) {
      throw new Error("Đéo tìm thấy URL âm thanh");
    }
...
export async function handleMusicCommand(api, message, aliasCommand) {
  ...
  try {
    ...
    if (!musicInfo || !musicInfo.collection || musicInfo.collection.length === 0) {
      const object = {
        caption: `Đéo tìm thấy bài hát nào với từ khóa: ${question}`,
      };
      await sendMessageWarningRequest(api, message, object, 30000);
      return;
    }
...
export async function handleSendTrackSoundCloud(api, message, track) {
  ...
  if (cachedMusic) {
    ...
    if (!url) {
      const object = {
        caption: `Xin lỗi, bé Đéo thể lấy được bài hát này về. Vui lòng thử lại bài khác.`,
      };
      await sendMessageWarningRequest(api, message, object, 30000);
      return true;
    }
