...
// Author: ndqitvn
// Description: ZingMP3 API rebuild by N C H
  ...
  try {
    ...
    if (url) {
      ...
      try {
        ...
        const object = {
          caption: `Link Đéo hợp lệ hoặc link thuộc thể loại album!`
    ...
    if (!result.data || !result.data.items || result.data.items.length === 0) {
      const object = {
        caption: `Đéo tìm thấy bài hát nào với từ khóa: ${keyword}`,
      };
      await sendMessageWarningRequest(api, message, object, 30000);
      return;
    }
...
export async function handleTopChartZingMp3(api, message, aliasCommand) {
  ...
  try {
    ...
    if (!result.data || !result.data.RTChart || !result.data.RTChart.items) {
      throw new Error("Đéo thể lấy được danh sách bài hát từ ZingMP3");
    }
...
export async function handleZingMp3Reply(api, message) {
  ...
  try {
    ...
    console.error(
      `Đéo thể lấy stream URL cho track ${track?.encodeId}:`,
      error
    );
    const object = {
      caption:
        `Đéo thể lấy bài này, vui lòng thử bài khác!\n` +
        `Nguyên Nhân: ${error.message}`,
    };
    await sendMessageWarningRequest(api, message, object, 30000);
    return true;
    ...
    switch (subCommand) {
      case "lyric":
        if (lyric.data && lyric.data.sentences) {
          ...
          await api.sendMessage(
            {
              msg: "Đéo tìm thấy lời cho bài hát này.",
              ttl: 30000,
            },
            message.threadId,
            message.type
