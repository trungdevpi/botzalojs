  ...
  if (!command || (command !== "add" && command !== "remove")) {
    if (Object.keys(groupSettings[threadId].whiteList || {}).length === 0) {
      await sendMessageWarning(
        api,
        message,
        "Hiện Đéo có người dùng nào trong danh sách white-list."
      );
    ...
    if (!isNaN(indexToRemove)) {
      ...
      if (indexToRemove > 0 && indexToRemove <= whiteListArray.length) {
        ...
        await sendMessageWarning(
          api,
          message,
          `Số thứ tự Đéo hợp lệ. Vui lòng chọn số từ 1 đến ${whiteListArray.length}.`
        );
        return false;
      }
    }
    ...
    if (command === "add") {
      if (isAdmin(userId, threadId)) {
        await sendMessageWarning(
          api,
          message,
          `${userName} đã là quản trị viên nên Đéo cần thêm vào white-list`
        );
        continue;
      }
      ...
      if (groupSettings[threadId].whiteList[userId]) {
        ...
        await sendMessageWarning(
          api,
          message,
          `${userName} Đéo có trong danh sách white-list.`
        );
      }
    }
  }
