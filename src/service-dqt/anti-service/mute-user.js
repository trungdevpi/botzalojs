...
export async function handleMuteList(api, message, groupSettings) {
  ...
  if (Object.keys(groupSettings[threadId].muteList).length === 0) {
    await sendMessageWarning(api, message, "Hiện Đéo có người dùng nào bị cấm chat.");
    return;
  }
  ...
  if (mentions && mentions.length > 0) {
    ...
    for (const mention of mentions) {
      ...
      if (isAdmin(userId, threadId, groupAdmins)) {
        await sendMessageWarning(api, message, `Đéo thể cấm chat ${userName} vì họ là quản trị viên`);
        continue;
      }
    ...
    for (const mention of unmuteReferences) {
      ...
      if (groupSettings[threadId].muteList[userId]) {
        ...
        await sendMessageWarning(
          api,
          message,
          `Người dùng ${userName.name || userName || userId} Đéo tồn tại trong danh sách cấm chat.`
        );
      }
    }
  } else {
