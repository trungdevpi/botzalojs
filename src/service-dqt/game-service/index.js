...
export async function checkBeforeJoinGame(api, message, groupSettings, checkLogin = false) {
  ...
  if (groupSettings) {
    ...
    if (activeGame === false) {
      ...
      if (isAdminBot) {
        text =
          "Trò chơi hiện tại Đéo được bật trong nhóm này.\n\n" +
...
export async function handleBuffCommand(api, message, groupSettings) {
  ...
  try {
    ...
    if (parsedAmount === "allin") {
      const result = {
        success: false,
        message: `Đéo thể sử dụng all/allin cho lệnh buff.`,
      };
...
export async function handleBankCommand(api, message, groupSettings) {
  ...
  if (new Big(requestData.balance).lt(bankAmount)) {
    const result = {
      success: false,
      message: `Số dư Đéo đủ. Bạn chỉ có ${formatBigNumber(new Big(requestData.balance))} VNĐ.`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return;
  }
  ...
  if (await isPlayerBanned(targetId)) {
    const result = {
      success: false,
      message: `${targetName} đã bị khóa tài khoản, Đéo thể chuyển tiền cho người dùng này.`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return;
  }
...
export async function handleBanCommand(api, message, groupSettings) {
  ...
  for (const mention of mentions) {
    ...
    if (isAdmin(targetId, threadId)) {
      const result = {
        success: false,
        message: `${targetName} là quản trị viên, Đéo thể khóa tài khoản được.`,
      };
      await sendMessageFromSQL(api, message, result, true, 300000);
      return;
    }
  ...
  for (const mention of mentions) {
    ...
    if (await isHaveLoginAccount(targetId)) {
      if (await isPlayerBanned(targetId)) {
        ...
        const result = {
          success: false,
          message: `${targetName} Đéo bị khóa tài khoản.`,
        };
        await sendMessageFromSQL(api, message, result, true, 300000);
      }
    } else {
...
export async function checkPlayerBanned(api, message, threadId, senderId) {
  if (await isPlayerBanned(senderId)) {
    const result = {
      success: false,
      message: `Tài khoản của bạn đã bị khóa, Đéo thể thực hiện bất kỳ lệnh game nào nữa!`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return true;
  }
...
export async function handleLoginPlayer(api, message, groupSettings) {
  ...
  if (message.type === MessageType.GroupMessage) {
    ...
    const result = {
      success: false,
      message: `Vì lý do bảo mật, bạn Đéo thể đăng nhập tài khoản trong nhóm!\nVui lòng nhắn riêng cho tôi để đăng nhập tài khoản.`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return;
  }
...
export async function handleRegisterPlayer(api, message, groupSettings) {
  ...
  if (await isHaveLoginAccount(senderId)) {
    const result = {
      success: false,
      message: `Bạn đã đăng nhập tài khoản game trên zalo này, Đéo thể đăng ký tài khoản mới.`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return;
  }
  ...
  if (message.type === MessageType.GroupMessage) {
    ...
    const result = {
      success: false,
      message: `Vì lý do bảo mật, bạn Đéo thể đăng ký tài khoản trong nhóm!\nVui lòng nhắn riêng cho tôi để đăng ký tài khoản.`,
    };
    await sendMessageFromSQL(api, message, result, true, 300000);
    return;
  }
...
export async function handleNapCommand(api, message, groupSettings) {
  try {
    ...
    if (accountVND === null) {
      const result = {
        success: false,
        message: `Đéo thể lấy thông tin số dư VND từ tài khoản game ${playerInfo.username}.`,
      };
      await sendMessageFromSQL(api, message, result, true, 300000);
      return;
    }
...
export async function handleRutCommand(api, message, groupSettings) {
  try {
    ...
    if (currentBotBalance.lt(rutAmount)) {
      const result = {
        success: false,
        message: `Số dư trong tài khoản bot Đéo đủ để rút ${formatBigNumber(rutAmount)} VNĐ về tài khoản ${playerInfo.username}!`,
      };
      await sendMessageFromSQL(api, message, result, true, 300000);
      return;
    }
