...
function checkJackpot(result, bets) {
  // Kiểm tra 3 con có giống nhau Đéo
  if (result[0] === result[1] && result[1] === result[2]) {
    const jackpotSymbol = result[0];
    // Kiểm tra người chơi có đặt cược vào con này Đéo
...
export async function handleBauCua(api, message, groupSettings) {
  ...
  if (requestData.success) {
    ...
    if (bigNumBalance.lt(totalBet)) {
      await api.sendMessage(
        { msg: `${nameServer}: ${status} Số dư Đéo đủ. Bạn chỉ có ${formatCurrency(bigNumBalance)} VNĐ.`, quote: message },
        threadId,
        message.type
      );
      return;
...
async function parseBets(content, currentBalance) {
  ...
  if (insufficientFunds) {
    throw new Error(`Số dư Đéo đủ. Bạn chỉ có ${formatCurrency(new Big(currentBalance))} VNĐ.`);
  }
...
async function createResultImage(result) {
  ...
  for (let i = 0; i < result.length; i++) {
    ...
    if (fs.existsSync(imagePath)) {
      ...
      console.error(`Đéo tìm thấy hình ảnh cho ${SYMBOL_NAMES[symbol]}`);
    }
  }
