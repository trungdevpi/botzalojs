...
export async function handleChanLe(api, message, groupSettings) {
  ...
  if (!playerChoice) {
    ...
    await api.sendMessage(
      {
        msg: `${nameServer}: Lựa chọn Đéo hợp lệ.\nCác cách nhập hợp lệ: ${validChoices}`,
        quote: message,
      },
      threadId,
      message.type
  ...
  if (new Big(balance.balance).lt(betAmount)) {
    await api.sendMessage(
      { msg: `${nameServer}: Số dư Đéo đủ. Bạn chỉ có ${formatCurrency(new Big(balance.balance))} VNĐ`, quote: message },
      threadId,
      message.type
    );
    return;
...
function normalizeChoice(choice) {
  ...
  // Chuyển đổi các ký tự có dấu thành Đéo dấu để so sánh
