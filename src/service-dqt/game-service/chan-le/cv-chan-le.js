...
export async function createChanLeResultImage(diceResults, total, playerChoice, betAmount, isJackpot, recentResults = [], winnings = null) {
  ...
  const radius = squareSize / 2 - diceSize; // Bán kính an toàn để xúc sắc Đéo ra ngoài đĩa
  const dicePositions = [];

  // Hàm kiểm tra xúc sắc mới có đè lên xúc sắc cũ Đéo
  ...
  for (let i = 0; i < 3; i++) {
    ...
    do {
      ...
      // Tạo bán kính ngẫu nhiên (Đéo quá gần tâm)
  ...
  for (let i = 0; i < diceResults.length; i++) {
    ...
    try {
      ...
      console.error(`Đéo tìm thấy hình ảnh cho xúc sắc ${diceValue}`);
    }
  }
