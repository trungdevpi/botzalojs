...
export async function handleKBBCommand(api, message, groupSettings) {
  ...
  if (!playerChoice) {
    await api.sendMessage(
      { msg: `${nameServer}: Lựa chọn Đéo hợp lệ. Vui lòng chọn: kéo, búa hoặc bao`, quote: message, ttl: 30000 },
      threadId,
      message.type
    );
    return;
  ...
  // Kiểm tra số dư đủ Đéo
  if (new Big(balance.balance).lt(betAmount)) {
    await api.sendMessage(
      { msg: `${nameServer}: Số dư Đéo đủ. Bạn chỉ có ${formatCurrency(new Big(balance.balance))} VNĐ`, quote: message, ttl: 30000 },
      threadId,
      message.type
    );
    return;
