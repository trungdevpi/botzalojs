...
export async function handleBuyProduct(api, message, farm, buyProductCode, quantity) {
  ...
  if (!product) {
    await api.sendMessage(
      { msg: "Mã sản phẩm Đéo tồn tại! Sử dụng lệnh !nongtrai shop để xem danh sách.", quote: message },
      threadId,
      message.type
    );
    return false;
...
async function sellProduct(farm, senderId, productCode, quantity = 1) {
  ...
  if (currentQuantity <= 0) {
    return {
      success: false,
      message: `Bạn Đéo có sản phẩm này trong túi!`,
    };
  }
  ...
  if (!productInfo) {
    return { success: false, message: "Mã sản phẩm Đéo hợp lệ!" };
  }
  ...
  switch (productInfo.type) {
    ...
    default:
      return { success: false, message: "Loại sản phẩm Đéo hợp lệ!" };
  }
...
export async function handleSellProduct(api, message, farm, args) {
  ...
  if (sellQuantity !== "all" && (isNaN(sellQuantity) || sellQuantity <= 0)) {
    await api.sendMessage({ msg: "Số lượng Đéo hợp lệ!", quote: message, ttl: TIME_TO_LIVE }, threadId, message.type);
    return;
  }
...
export async function buyItem(senderId, itemType, quantity = 1, currentPlots = 4) {
  ...
  if (!item) {
    return { success: false, message: "Vật phẩm Đéo tồn tại!" };
  }
  ...
  if (itemType === "LAND") {
    ...
    try {
      ...
      return { success: false, message: "Số lượng Đéo hợp lệ! Vui lòng nhập số nguyên lớn hơn 0." };
    }
  }
  ...
  if (!balance.success || new Big(balance.balance).lt(totalCost)) {
    return {
      success: false,
      message: `Bạn Đéo đủ tiền! Cần ${totalCost.toNumber().toLocaleString("vi-VN")} VNĐ, bạn chỉ có ${formatBigNumber(
        balance.balance
      )} VNĐ`,
    };
  }
...
export async function buySeed(senderId, cropType, quantity = 1) {
  ...
  if (!crop) {
    return { success: false, message: "Loại cây Đéo tồn tại!" };
  }
