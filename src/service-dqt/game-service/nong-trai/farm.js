...
async function digSoil(farm, plotIndex) {
  ...
  // Kiểm tra ô đất có tồn tại Đéo
  if (!plot) {
    return { success: false, message: "Ô đất Đéo tồn tại!", plotIndex };
  }
  ...
  // Kiểm tra có cuốc trong túi đồ Đéo
  if (!farm.bag["S1"] || farm.bag["S1"] <= 0) {
    return { success: false, message: "Bạn Đéo có cuốc trong túi đồ! Hãy mua cuốc từ shop.", plotIndex };
  }
...
async function waterPlot(farm, plotIndex) {
  ...
  if (currentWaterLevel >= 100) {
    return {
      success: false,
      message: "Độ ẩm đã đạt tối đa (100%), Đéo cần tưới thêm nước!",
      plotIndex,
    };
  }

  // Kiểm tra có bình nước trong túi Đéo
  if (!farm.bag["W1"] || farm.bag["W1"] <= 0) {
    return {
      success: false,
      message: "Bạn Đéo có bình nước trong túi đồ! Hãy mua bình nước (W1) từ shop.",
      plotIndex,
    };
  }
  ...
  const targetWaterLevel = Math.min(100, currentWaterLevel + 60); // Tăng thêm 60% nhưng Đéo vượt quá 100%
...
async function plantCrop(farm, plotIndex, cropCode, senderId) {
  ...
  if (!productInfo || productInfo.type !== "CROP") {
    return { success: false, message: "Mã hạt giống Đéo hợp lệ!", plotIndex };
  }

  const bag = farm.bag;
  if (!bag[cropCode] || bag[cropCode] <= 0) {
    return { success: false, message: "Bạn Đéo có hạt giống này trong túi đồ!", plotIndex };
  }
...
export async function useItem(plot, itemType, farm, plotIndex, senderId) {
  ...
  if (!item) {
    return { success: false, message: "Vật phẩm Đéo tồn tại!", plotIndex };
  }
  ...
  switch (itemType) {
    ...
    case "SUPER_FERTILIZER":
      if (!plot.crop) {
        return { success: false, message: "Đéo có cây để bón phân!", plotIndex };
      }
      ...
      if (timeLeft.lte(0)) {
        return { success: false, message: "Cây đã sẵn sàng thu hoạch, Đéo thể bón phân!", plotIndex };
      }
    ...
    default:
      return { success: false, message: "Đéo thể sử dụng vật phẩm này!", plotIndex };
  }
}
...
export async function handleUseItemCommand(api, message, farm, args) {
  ...
  if (!useProduct) {
    await api.sendMessage({ msg: "Mã sản phẩm Đéo hợp lệ!", quote: message, ttl: TIME_TO_LIVE }, threadId, message.type);
    return;
  }
  ...
  if (itemQuantity < plotIndexesToUse.length) {
    ...
    await api.sendMessage(
      { msg: `Bạn chỉ có ${itemQuantity} ${itemName}, Đéo đủ để dùng cho ${plotIndexesToUse.length} ô!`, quote: message },
      threadId,
      message.type
    );
    return;
...
export async function handleGieoHatCommand(api, message, farm, args) {
  ...
  if (args.length >= 2) {
    ...
    const productInfo = PRODUCT_CODES[cropCode];
    if (!productInfo || productInfo.type !== "CROP") {
      await api.sendMessage({ msg: "Mã hạt giống Đéo hợp lệ!", quote: message }, threadId, message.type);
      return;
    }
    ...
    if (seedCount < plotIndexes.length) {
      await api.sendMessage(
        {
          msg: `Bạn chỉ có ${seedCount} hạt giống ${CROPS[productInfo.key].name}, Đéo đủ để trồng ${plotIndexes.length} ô!`,
          quote: message,
        },
        threadId,
        message.type
    ...
    // Nếu Đéo có đối số thì hiển thị danh sách hạt giống trong túi
    const seeds = [];
...
async function harvestCrop(farm, plotIndex, senderId) {
  ...
  if (!plot.crop) {
    return { success: false, message: "Đéo có cây để thu hoạch!", plotIndex };
  }
