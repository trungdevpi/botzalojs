  ...
  if (Object.keys(currentSession.players).length > 0) {
    ...
    if (jackpotWinners.length > 0) {
      ...
      for (const winner of jackpotWinners) {
        ...
        // Giới hạn tiền thưởng Đéo vượt quá 1000% số tiền cược
    ...
    resultText += "Đéo có người chơi trong phiên này.\n";
  }
...
async function placeBet(api, message, threadId, senderId, betType, amount) {
  if (!activeThreads.has(threadId)) {
    await sendMessageFromSQL(
      ...
      {
        success: false,
        message: "Trò chơi Tài Xỉu Đéo được bật trong nhóm này.",
      },
      true,
      30000
    );
  ...
  if (!balanceResult.success) {
    const result = {
      success: false,
      message: "Đéo thể lấy thông tin số dư. Vui lòng thử lại sau\nNếu chưa đăng ký, chat game để xem hớng dẫn.",
    };
    await sendMessageFromSQL(api, message, result, true, 30000);
    return;
  }
...
export function setForcedResult(result) {
  if (result !== "tai" && result !== "xiu") {
    throw new Error("Kết quả Đéo hợp lệ. Chỉ chấp nhận 'tai' hoặc 'xiu'.");
  }
...
Tổng điểm: ${forcedResult.total}`;
        ...
        await api.sendMessage(
          { msg: "Lệnh Đéo hợp lệ. Sử dụng '!tx kq tai' hoặc '!tx kq xiu'.", ttl: 30000 },
          threadId,
  ...
  if (betMatch) {
    ...
    const result = {
      success: false,
      message:
        "Lệnh Đéo hợp lệ. Sử dụng lệnh sau để tham gia trò chơi:\n" +
